...
export function AnalyticsCharts({ logs, totalHours, targetHours }: AnalyticsChartsProps) {
  const chartData = useMemo(() => {
    if (logs.length === 0) return { weeklyData: [], dailyTrend: [] };
    ...
    const dailyTrend = sortedLogs.slice(-14).map((log) => ({
    ...
    return { weeklyData, dailyTrend };
  }, [logs]);

  const stats = useMemo(() => {
    if (logs.length === 0) {
      return { avgDaily: 0, maxDay: 0, totalDays: 0, weeksActive: 0 };
    }
